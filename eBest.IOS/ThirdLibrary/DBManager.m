...
//插入
- (void)insertDataModel:(DetailModel *)model{
    ...
    BOOL success=[_dataBase executeUpdate:insertSql,model.dataID,model.goods_name,model.goods_main_photo_path,model.goods_current_price,model.goods_price];
    NSLog(@"%@",model.goods_name);
    NSLog(@"%@",model.goods_main_photo_path);
    if (!success) {
        NSLog(@"%@",[_dataBase lastErrorMessage]);
...
- (NSArray *)getData{
    ...
    while ([set next]) {
      DetailModel* model = [[DetailModel alloc]init];
        ...
        NSLog(@"%@",model.dataID);
        model.goods_name = [set stringForColumn:@"title"];
        NSLog(@"%@",model.goods_name);
        model.goods_main_photo_path =[set stringForColumn:@"pic"];
        NSLog(@"%@",model.goods_main_photo_path);
        model.goods_current_price=[set stringForColumn:@"createtime"];
        NSLog(@"%@",model.goods_current_price);
        model.goods_price =[set stringForColumn:@"author"];
