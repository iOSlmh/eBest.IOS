...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    self.tabBarController.tabBar.bounds = CGRectMake(0, self.view.frame.origin.y - 64, self.view.frame.size.width, 42);
...
{
    NSUserDefaults * kefu = [NSUserDefaults standardUserDefaults];

    for (EMMessage *message in aMessages) {
        //客服信息
        NSLog(@"----------------%@",message.ext);
        NSString * kefuNick = message.ext[@"weichat"][@"agent"][@"userNickname"];
        NSString * kefuPath = message.ext[@"weichat"][@"agent"][@"avatar"];
        NSString * kefuHead = [NSString stringWithFormat:@"%@%@",Base_hx,kefuPath];
        [kefu setObject:kefuNick forKey:@"kefuNick"];
        [kefu setObject:kefuHead forKey:@"kefuHead"];
