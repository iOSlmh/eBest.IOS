...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        ...
        [clearButton setImage:[UIImage imageNamed:@"button_delete photos"] forState:UIControlStateNormal];
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    if (model.isSender) {
        ...
        NSUserDefaults * headuser = [NSUserDefaults standardUserDefaults];
        NSString *headStr= [headuser objectForKey:@"headImage"];
        model.avatarURLPath = headStr;

        //model.nickname= MYnickName;
    }else{
        //客服头像
        
        NSUserDefaults * headuser = [NSUserDefaults standardUserDefaults];
        NSString *headStr= [headuser objectForKey:@"kefuNick"];
        NSString * kefuHead = [headuser objectForKey:@"kefuHead"];
        NSString *encodedString = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)kefuHead, (CFStringRef)@"!NULL,'()*+,-./:;=?@_~%#[]", NULL, kCFStringEncodingUTF8));
        model.avatarURLPath = encodedString;
        model.nickname = headStr;
...
- (void)deleteAllMessages:(id)sender
{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"确定清空消息记录？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alertView show];
    }
}
