...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    [_photoButton setImage:[UIImage imageNamed:@"danliao相册"] forState:UIControlStateNormal];
    [_photoButton setImage:[UIImage imageNamed:@"danliao相册"] forState:UIControlStateHighlighted];
    [_photoButton addTarget:self action:@selector(photoAction) forControlEvents:UIControlEventTouchUpInside];
    _photoButton.tag = MOREVIEW_BUTTON_TAG;
    UILabel * zhaopianL = [[UILabel alloc]init];
    zhaopianL.frame =CGRectMake(insets, 10+5+CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15);
    zhaopianL.text =@"图片";
    zhaopianL.font = [UIFont systemFontOfSize:15];
    zhaopianL.textColor = [UIColor blackColor];
    zhaopianL.textAlignment =  NSTextAlignmentCenter;
    [_scrollview addSubview:zhaopianL];
    [_scrollview addSubview:_photoButton];
    
    _locationButton =[UIButton buttonWithType:UIButtonTypeCustom];
    [_locationButton setFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    [_locationButton setImage:[UIImage imageNamed:@"danliao拍照"] forState:UIControlStateNormal];
    [_locationButton setImage:[UIImage imageNamed:@"danliao拍照"] forState:UIControlStateHighlighted];
    [_locationButton addTarget:self action:@selector(locationAction) forControlEvents:UIControlEventTouchUpInside];
    _locationButton.tag = MOREVIEW_BUTTON_TAG + 1;
    UILabel * paizhaoL = [[UILabel alloc]init];
    paizhaoL.frame =CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 10+5+CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15);
    paizhaoL.text =@"拍照";
    paizhaoL.font = [UIFont systemFontOfSize:15];
    paizhaoL.textColor = [UIColor blackColor];
    paizhaoL.textAlignment =  NSTextAlignmentCenter;
    
    [_scrollview addSubview:paizhaoL];
    ...
    [_takePicButton setImage:[UIImage imageNamed:@"danliao位置"] forState:UIControlStateNormal];
    [_takePicButton setImage:[UIImage imageNamed:@"danliao位置"] forState:UIControlStateHighlighted];
    [_takePicButton addTarget:self action:@selector(takePicAction) forControlEvents:UIControlEventTouchUpInside];
    _takePicButton.tag = MOREVIEW_BUTTON_TAG + 2;
    _maxIndex = 2;
    UILabel * dingweiL = [[UILabel alloc]init];
    dingweiL.frame =CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 10+5+CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15);
    dingweiL.text =@"位置";
    dingweiL.font = [UIFont systemFontOfSize:15];
    dingweiL.textColor = [UIColor blackColor];
    dingweiL.textAlignment =  NSTextAlignmentCenter;
    
    [_scrollview addSubview:dingweiL];
    ...
    if (type == EMChatToolbarTypeChat) {
        ...
        [_videoCallButton setFrame:CGRectMake(insets, 10 * 2 + CHAT_BUTTON_SIZE + 20, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
