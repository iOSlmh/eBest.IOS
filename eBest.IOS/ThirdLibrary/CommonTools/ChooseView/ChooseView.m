...
@synthesize alphaView,whiteView,headImage,LB_detail,LB_line,LB_price,LB_stock,LB_showSales,mainscrollview,cancelBtn,confirmBtn,stockBtn;
...
-(void)creatUI{
    ...
    //半透明视图
    alphaView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SW, SH)];
    alphaView.backgroundColor = [UIColor blackColor];
    alphaView.alpha = 0.5;
    [self addSubview:alphaView];
    
    //装载商品信息的视图
    whiteView = [[UIView alloc] initWithFrame:CGRectMake(0, 0.3*(SH-48-64), SW, 0.7*(SH-48-64))];
    whiteView.backgroundColor = [UIColor whiteColor];
    [self addSubview:whiteView];
    
    //商品图片
    headImage = [[UIImageView alloc] initWithFrame:CGRectMake(40, -30, 100, 100)];
    headImage.image = [UIImage imageNamed:@"1"];
    headImage.layer.cornerRadius = 10;
    headImage.layer.borderColor = RGB(32, 179, 169, 1).CGColor;
    headImage.layer.borderWidth = 1;
    [headImage.layer setMasksToBounds:YES];
    [whiteView addSubview:headImage];
    
    cancelBtn= [UIButton buttonWithType:UIButtonTypeCustom];
    cancelBtn.frame = CGRectMake(SW-34, 16, 18, 18);
    [cancelBtn setImage:[UIImage imageNamed:@"nav_close"] forState:0];
...
//    [whiteView addSubview:LB_showSales];
    ...
    //分界线
    LB_line = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headImage.frame)+10, SW, 0.5)];
    LB_line.backgroundColor = BackgroundColor;
    [whiteView addSubview:LB_line];
    
    //加入购物车按钮
    confirmBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    confirmBtn.frame = CGRectMake(0, whiteView.height-40, SW, 40);
    
    [confirmBtn setBackgroundColor:RGB(42, 150, 143, 1)];
    ...
    stockBtn.frame = CGRectMake(0,  whiteView.height-40, SW, 40);
    ...
    mainscrollview = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headImage.frame)+10, SW, whiteView.height-CGRectGetMaxY(headImage.frame)+10-60)];
